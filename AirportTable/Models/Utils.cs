    ...
    public static class Utils
    {
        ...
        public static string Obj2json(object? obj)
        {
            switch (obj)
            {
                ...
                case Point @point:
                    return "\"$p$" + (int)@point.X + "," + (int)@point.Y + '"';

                case Points @points:
                    return "\"$P$" + string.Join("|", @points.Select(p => (int)p.X + "," + (int)p.Y)) + '"';

                case SolidColorBrush @color:
                    return "\"$C$" + @color.Color + '"';

                case Thickness @thickness:
                    return "\"$T$" + @thickness.Left + "," + @thickness.Top + "," + @thickness.Right + "," + @thickness.Bottom + '"';
        ...
        private static object JsonHandler(string str)
        {
            ...
            return str[1] switch
            {
                ...
                'T' => new Thickness(double.Parse(thick[0]), double.Parse(thick[1]),
                                     double.Parse(thick[2]), double.Parse(thick[3])),
                _ => str,
            };
        }
        private static object? JsonHandler(object? obj)
        {
            ...
            if (obj is object?[] @list)
                return @list.Select(JsonHandler).ToArray();
        ...
        private static bool IsComposite(object? obj)
        {
            ...
            if (obj is List<object?> ||
                ...
                obj is not JsonElement @item)
                return true;

            var T = @item.ValueKind;
            return T == JsonValueKind.Object ||
                   T == JsonValueKind.Array;
        }
        private static string Dict2XML(Dictionary<string, object?> dict, string level)
        {
            ...
            foreach (var entry in dict)
                if (IsComposite(entry.Value))
                    items.Append(level + "\t<" + entry.Key + ">" +
                                 ToXMLHandler(entry.Value, level + "\t\t") +
                                 level + "\t</" + entry.Key + ">");
                else
                    attrs.Append(" " + entry.Key + "=\"" +
                                 ToXMLHandler(entry.Value, "{err}") + "\"");

            if (items.Length == 0)
                return level + "<Dict" + attrs.ToString() + "/>";

            return level + "<Dict" + attrs.ToString() +
                   ">" + items.ToString() + level + "</Dict>";
        }
        ...
        private static string Dict2YAML(Dictionary<string, object?> dict, string level)
        {
            ...
            foreach (var entry in dict)
            {
                res.Append(level + YAMLEscape(entry.Key) +
                    ":" + (IsComposite(entry.Value) ? "" : " ")
                    + ToYAMLHandler(entry.Value, level + "\t"));
            }
        ...
        private static string List2YAML(List<object?> list, string level)
        {
            ...
            StringBuilder res = new();
            foreach (var entry in list)
                res.Append(level + "-" + (IsComposite(entry) ? "" : " ")
                    + ToYAMLHandler(entry, level + "\t"));
